...
l = 0.3 # Distance between two wheels in same direction
...
for i in range(len(t)):
    ...
    # input
    omega1 = 0.3*t[i]
    omega2 = 0.5*np.tan(0.1*t[i])
    omega3 = 3*np.log(1+0.1*t[i])
    omega4 = 5*np.sin(0.1*t[i])
    omega = np.array([omega1,omega2,omega3,omega4]).reshape(4,1)

    # wheel configuration
    W = np.array([
        [a/2,a/2,a/2,a/2],
        [0,0,0,0],
        [-a/(4*d),-a/(4*d),a/(4*d),a/(4*d)]
    ])
